"""
Adapted from https://github.com/RaphaelOlivier/whisper_attack/blob/main/loss.py
Olivier, R. and Raj, B., “There is more than one kind of robustness: Fooling Whisper with adversarial examples”, https://doi.org/10.48550/arXiv.2210.17316

Changes:
    - In line 96, changed `torch.tensor(label)` to `label.detach().clone()` to address UserWarning and removed `.repeat(n_audio, 1)` to allow multiple different labels
    - Commented out lines 93-94 to disallow passing strings as labels
    - Added lines 56-57 to ignore padding tokens in loss calculation:
        `corrected_tokens = torch.where(tokens == self.tokenizer.eot, -100, tokens)`
        `corrected_tokens[:, -1] = self.tokenizer.eot`
    - In lines 59 and 61, changed references to `tokens` to `corrected_tokens`
    - In line 72, changed `torch.nn.CrossEntropyLoss(reduction="none")` to `torch.nn.CrossEntropyLoss(reduction="none", ignore_index=-100)` to be explicit
"""
...
class LossTask(DecodingTask):
    ...
    def _loss_from_logits(self, logits, tokens, loss_fct):
        ...
        loss = loss_fct(logits.transpose(1, 2), corrected_tokens).mean(dim=1)
        if self.correct_first_word:
            corrective_first_word_loss = loss_fct(logits[:, 0], corrected_tokens[:, 0])
    ...
    def run(self, mel: Tensor, label: Union[str, torch.Tensor]) -> List[LossResult]:
        ...
        input_tokens: Tensor = label.detach().clone().to(audio_features.device)
